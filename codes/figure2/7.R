...
t=parSapply(cl,1:150,function(X){
  library(parallel)
  library(stats)
  library(fcd)
  label=function(x){
  ...
  int.x=function(df,K){
    ...
    for (i in 1:K){
      ...
      para=optim(c(0,2),model,df=df1,gm=gm,
  ...
  p.noise.x=0.7; p.noise.A=seq(0.1,0.9,length=9);K=4
  p=1000; n=40; 
  m.x=rmultinom(p, size = 1, prob = c(0.5,0.5))
  l.x=apply(m.x,2,label); para1=list(2,0.5); para2=list(4,0.5)
  m.A=rmultinom(p, size = 1, prob = c(0.5,0.5)); l.A=apply(m.A,2,label)
  P=matrix(c(0.1,0.06,0.06,0.1),2,2); p_0=0.0001
  ...
  u=rep(0,p)
  for (i in 1:p){
    if (l.x[i]==1) u[i]=rnorm(1,para1[[1]],para1[[2]])
    else {u[i]=rnorm(1,para2[[1]],para2[[2]])}
  }
  se=runif(p,0.2,1.5)
  df=matrix(0,p,n)
  ...
  for (m in 1:M){
      ...
      for (i in 1:(p-1)){
        for (j in (i+1):p){
          if (rbinom(1,1,1-p.noise.A[m])==1){
            if (rbinom(1,1,P[l.A[i],l.A[j]])==1){A.noise[i,j]=1}
          }
          else {
    ...
    label=l
    K=4; e_int=spectral.clustering(A.noise,K=K)
    r=tryCatch(IntegrativeEM(A.noise,e_int,df.noise,int.x(df.noise,K)$para_int),error=function(e){return(NaN)})
