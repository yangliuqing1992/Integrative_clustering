...
# }
for (i in 1:p){
  u[i]=rnorm(1, para[[label_true[i]]][[1]], para[[label_true[i]]][[2]])
}
se=runif(p,0.2,1.5)
df=matrix(0,p,n)
...
for (i in 1:(p-1)){
  for (j in (i+1):p){
    if (rbinom(1,1,1-p.noise.A)==1){
      if (rbinom(1,1,P[label_true[i],label_true[j]])==1){A.noise[i,j]=1}
    }
    else {
...
w.2=matrix(1,p,n)
r.X = tryCatch(int.x(df.noise,K),error=function(e){return(NaN)})


#r.AX=tryCatch(IntegrativeEM(A.noise,e,df.noise,x_initials$para_int),error=function(e){return(NaN)})
...
for (k in 1:K)
  for (l in 1:K)
    P[k,l]=mean(A.noise[r.A$e==k,r.A$e==l])
...
se=apply(df.noise,1,sd)

for (k in 1:K)
{
  para[[k]][2]=sqrt(1/para[[k]][2])
}

save(prob=prob,para=para,P=P,se=se,file="parameters.RData")
matchLabel(label_true,r.X$cl)$adjRand
