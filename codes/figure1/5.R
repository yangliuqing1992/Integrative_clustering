...
t=parSapply(cl,1:200,function(X){
  library(parallel)
  library(stats)
  library(fcd)
  label=function(x){
  ...
  { 
    p_e=matrix(0,length(sample[,1]),3)
  ...
  int.x=function(df,K){
    ...
    cl=kmeans(df.input,K)$cluster; p_int=rep(0,3); para_int=list()
  ...
  p.noise.x=0.5; p.noise.A=seq(0.1,0.9,length=9);K=3
  p=1000; n=40; 
  m=rmultinom(p, size = 1, prob = c(0.3,0.3,0.4))
  l=apply(m,2,label); para1=list(-0.5,0.6); para2=list(2,0.6); para3=list(4,0.8)
  P=matrix(c(0.12,0.06,0.06,0.06,0.12,0.06,0.06,0.06,0.12),3,3); p_0=0.0001
  u=rep(0,p)
  for (i in 1:p){
